...
const logger = pino();
const accessToken = await getServiceAccountJwt(
...
test("should be able to get balances", async () => {
  const balances = await algo724Client.client.GET(
...
test("should be able to get quotes priced for different quantities and order sides", async () => {
  ...
  const trade2 = await algo724Client.client.POST(
  ...
  const trade3 = await algo724Client.client.POST(
    "/api/v2/trading/fetchQuotes",
    {
      body: {
        orderSide: "BUY",
...
test(`Quote and execute 1 trades at the same time`, async () => {
  ...
  eventEmitter.on("quote", async (o) => {
    const quote = await algo724Client.client.POST(
      "/api/v2/trading/fetchQuotes",
      {
        body: {
          orderSide: "BUY",
